...
function setSystemStatus(level, msg = "") {
	...
	if (level === "ERROR") {
		...
		if (dot) dot.className = "w-2.5 h-2.5 rounded-full bg-red-500 breathing-light-red";
		if (text) {
			text.innerText = "異常狀態";
		...
		if (dot) dot.className = "w-2.5 h-2.5 rounded-full bg-emerald-400 breathing-light";
		if (text) {
...
function handleCameraError(customMsg) {
	...
	isCameraError = true;
	const msg = typeof customMsg === "string" ? customMsg : "⚠️ 相機讀取失敗或尚未授權";
...
function toggleVoice() {
	...
	if (!isMuted) {
		...
		if (isCameraError && warningSound && warningSound.paused) {
			warningSound.play().catch((e) => console.log("警告音效播放失敗", e));
		}
	} else {
	...
	if ("Notification" in window) {
		if (Notification.permission !== "granted" && Notification.permission !== "denied") {
			Notification.requestPermission().then((permission) => {
				if (permission === "granted") console.log("✅ 系統通知權限已開啟");
			});
		}
	}
});
...
socket.on("state_update", function (state) {
	...
	if (state.light === "TOO DARK" && state.user_absent && !isCameraHidden) {
		...
		if (secondsPassed > 1.0 && secondsPassed <= 4.0) {
			...
			setSystemStatus("WARNING", `⚠️ 畫面異常，確認相機中... ${countdown}s`);
	...
	if (state.mode === "WORK") {
		...
		if (state.light === "TOO DARK" && !isCameraError) alertDetails.push("環境光源不足");
	...
	if (workBar) {
		workBar.style.width = Math.min((state.work_time / state.target_work_time) * 100, 100) + "%";
	}
	...
	if (container) {
		...
		let currentRenderedCount = parseInt(container.getAttribute("data-count") || "-1");
	...
	// 主人不在覆蓋層
	document.getElementById("overlay-absent")?.classList.toggle("hidden", !state.user_absent || isCameraHidden || isCameraError);
	...
	if (state.mode === "CALIBRATION" && !isCameraHidden && !isCameraError) {
		...
		if (state.calibration_status === "POSTURE_BAD") {
			...
			if(calibIcon) calibIcon.className = "fa-solid fa-triangle-exclamation text-6xl text-red-500 mb-8 animate-bounce";
			if(calibTitle) {
			...
			if (calibBar) calibBar.classList.replace("bg-purple-400", "bg-red-400");
			...
			if(calibIcon) calibIcon.className = "fa-solid fa-crosshairs text-6xl text-purple-400 mb-8 animate-spin";
			if(calibTitle) {
			...
			if (calibBar) calibBar.classList.replace("bg-red-400", "bg-purple-400");
	...
	function update(cid, vid, isAlert, text) {
		...
		if (isAlert) {
			...
			v.innerText = (isCameraError || currentSystemLevel === "WARNING") ? "---" : text;
			if (icon) {
...
function setPomodoro(seconds, btn, isHeader = false) {
    isSettled = false; // 🌟 設定新時間時，清除結算的溫暖字樣
	...
	headerButtons.forEach((b) => {
		b.className = "text-xs font-bold text-slate-500 px-3 py-1.5 rounded-full hover:bg-white transition-all";
	});

	if (btn) {
		if (isHeader) {
			btn.className = "btn-active text-xs font-bold text-white bg-purple-400 px-3 py-1.5 rounded-full transition-all shadow-sm";
			if (seconds === 1500) {
				const modal25 = document.getElementById("btn-mode-25");
				if (modal25) {
					modal25.className = "btn-active text-sm font-bold text-purple-700 px-4 py-2.5 rounded-xl border border-purple-200 transition-all flex-grow text-center shadow-sm";
					const span = modal25.querySelector("span");
					if (span) span.className = "text-[10px] text-purple-600/80 font-normal";
				}
			}
			if (seconds === 10) {
				const modal10 = document.getElementById("btn-mode-10");
				if (modal10) {
					modal10.className = "btn-active text-sm font-bold text-purple-700 px-4 py-2.5 rounded-xl border border-purple-200 transition-all flex-grow text-center shadow-sm";
					const span = modal10.querySelector("span");
					if (span) span.className = "text-[10px] text-purple-600/80 font-normal";
				}
			}
		} else {
			btn.className = "btn-active text-sm font-bold text-purple-700 px-4 py-2.5 rounded-xl border border-purple-200 transition-all flex-grow text-center shadow-sm";
			const span = btn.querySelector("span");
			if (span) span.className = "text-[10px] text-purple-600/80 font-normal";

			if (seconds === 1500) {
				const header25 = document.getElementById("btn-header-25m");
				if (header25) header25.className = "btn-active text-xs font-bold text-white bg-purple-400 px-3 py-1.5 rounded-full transition-all shadow-sm";
			}
			if (seconds === 10) {
				const header10 = document.getElementById("btn-header-10s");
				if (header10) header10.className = "btn-active text-xs font-bold text-white bg-purple-400 px-3 py-1.5 rounded-full transition-all shadow-sm";
			}
		}
...
socket.on("cloud_data_response", function (res) {
	...
	if (!rawData || rawData.length === 0) {
		container.innerHTML = "<p class='text-center text-slate-500 mt-10 font-bold'>目前沒有任何雲端資料喔！</p>";
		return;
	}
	...
	rawData.forEach(row => {
		...
		if (!aggregatedData[dateKey]) {
			aggregatedData[dateKey] = { date: row.date, work_time: 0, eyes: 0, shoulders: 0, light: 0 };
		}
	...
	const mergedArray = Object.values(aggregatedData).sort((a, b) => new Date(a.date) - new Date(b.date));
	container.innerHTML = "";
	...
	reversedData.forEach(row => {
		...
		let batteryStr = batteryCount > 0 
            ? "🔋".repeat(Math.min(batteryCount, 12)) + (batteryCount > 12 ? "+" : "") 
            : "🪫 <span class='text-xs text-slate-400'>(不到半小時)</span>";

		let starbucksCount = Math.floor(workMins / 60);
		let starbucksStr = starbucksCount > 0
            ? "☕".repeat(Math.min(starbucksCount, 12)) + (starbucksCount > 12 ? "+" : "")
            : "💸 <span class='text-xs text-slate-400'>(還不夠買一杯)</span>";

		let eyesStr = row.eyes > 0 
            ? "💧".repeat(Math.min(row.eyes, 12)) + (row.eyes > 12 ? "+" : "") 
            : "✨ <span class='text-xs text-emerald-500'>(無疲勞)</span>";

		let shouldersStr = row.shoulders > 0 
            ? "🧱".repeat(Math.min(row.shoulders, 12)) + (row.shoulders > 12 ? "+" : "") 
            : "✨ <span class='text-xs text-emerald-500'>(姿勢端正)</span>";

		let lightStr = row.light > 0 
            ? "🕯️".repeat(Math.min(row.light, 12)) + (row.light > 12 ? "+" : "") 
...
function sendReport(type) {
	...
	if (!currentWebhook.startsWith("http")) {
		openWebhookPromptModal(type === "daily" ? "report_daily" : "report_weekly");
		return; 
	}
...
socket.on("report_status", function (data) {
	...
	setTimeout(() => {
		document.getElementById("report-text-" + rtype).innerText = rtype === "daily" ? "發日報" : "發週報";
...
function confirmSettle() {
	const syncDiscord = document.getElementById("discord-sync-checkbox").checked;
	const currentWebhook = localStorage.getItem("discordWebhookUrl") || "";
...
function openWebhookPromptModal(actionType) {
	...
	if (actionType === "settle") {
		descText.innerText = "您勾選了發送報告，但尚未設定 Discord Webhook 網址喔！請在此補填：";
