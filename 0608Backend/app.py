...
# ==========================================
# 🚀 深度 AI 數據洞察生成器 (NVIDIA)
# ==========================================
def get_ai_advice(time_str, alerts, history=None):
    if not nvidia_client:
        return "💡 *系統提示：設定 NVIDIA API Key 後，即可解鎖專屬 AI 深度分析功能！*"
    try:
        trend_context = "無過去幾天數據，請僅基於今日表現進行單日健康風險評估。"
        if history and len(history) > 0:
            trend_context = "【過去幾天歷史趨勢參考】\n"
            for i, day in enumerate(history[-3:]): 
                trend_context += f"前 {len(history)-i} 天: 專注 {int(day['time']//60)} 分鐘, 疲勞 {day['alerts']['eyes']}次, 高低肩 {day['alerts']['shoulders']}次, 哈欠 {day['alerts']['mouth']}次\n"

        prompt = f"""
        你是一位受過專業醫學與人體工學訓練的「行為數據分析師」。請根據使用者的【今日數據】與【歷史趨勢】，進行 50 到 100 字的深度診斷。
        
        ⚠️ 嚴格限制：
        1. 絕對不要只是把數字重述一次。
        2. 必須進行「趨勢分析」。
        3. 給出「一個」最核心、具體可執行的硬體或姿勢調整建議。
        4. 語氣要客觀、專業、具備洞察力。
        ...
        【今日數據】
        - 專注總時長：{time_str}
        - 眼部疲勞(閉眼)：{alerts['eyes']} 次
        - 肩膀歪斜(高低肩)：{alerts['shoulders']} 次
        - 距離螢幕太近：{alerts['dist']} 次
        - 精神不濟(打哈欠)：{alerts['mouth']} 次
        - 環境光線太暗：{alerts['light']} 次
        """
        ...
        return f"🧠 **PulseAI 深度數據洞察 (Powered by NVIDIA)：**\n> {advice_text}"
        
    except Exception as e:
        return "💡 *AI 洞察分析產生中發生錯誤，請稍後再試。*"
...
def _discord_task(report_type, is_manual):
    ...
    try:
        if report_type == "daily":
            ...
            ai_advice = get_ai_advice(time_str, alerts, history=hist_snap)
            ...
            ai_advice = get_ai_advice(time_str, total_alerts, history=hist_snap)
                            ...
                            if current_mode == "CALIBRATION":
                                ...
                                if is_valid_calib:
                                    ...
                                    if calib_idx >= CALIBRATION_FRAMES:
                                        ...
                                        thresholds["ear"] = m_ear - max(3 * s_ear, 0.03) 
                                        thresholds["mar"] = m_mar + max(5 * s_mar, 0.15) 
