const ROOM_STORAGE_KEY = "pulseai_room_id";
...
setInterval(() => {
	...
	if (timeSinceLastData > 1000 && timeSinceLastData <= 4000) {
		...
		handleCameraError("⚠️ 瀏覽器鏡頭或 AI 推論已中斷");
	}
}, 500);
...
function triggerRecalibrate(btn) {
	window.pulseVision?.recalibrate();
...
document.addEventListener("DOMContentLoaded", () => {
	...
	window.startPulseVision?.(socket).catch((error) => {
		console.error(error);
		handleCameraError(
